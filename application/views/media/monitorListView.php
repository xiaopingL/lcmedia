...
<?php echo $this->load->view('media/tab'); ?>
...
<form method="post" action="">
    <thead>
        <tr>
            ...
            <th><a href="<?php echo site_url("media/MonitorController/monitorAddView/?type=".$type)?>">新建信息</a></th>
        </tr>
    </thead>
</form>
</table>
        ...
        <tbody>
           <?php if(!empty($result)){ foreach($result as $value){ ?>
            <tr class="rowlink">
                ...
                <td>
                    <?php if($this->session->userdata('uId')==$value['operator']):?><a href="javascript:deleteConFirm('<?php echo site_url("/media/MonitorController/monitorDel/".$value['rId']); ?>')" class="sepV_a" title="删除"><i class="icon-trash"></i></a><?php endif; ?>
                </td>
            </tr>
            <?php }}else{ ?>
